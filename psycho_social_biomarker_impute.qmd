...
koos <- koos %>%
  ...
  mutate(
    new_pain_score = case_when(
      pain_not_na >= 2 ~ 100 -
        ((rowMeans(select(., all_of(pain)), na.rm = TRUE) * 100) / 4)
    )
  ) %>%
  mutate(
    new_func_score = case_when(
      func_not_na >= 2 ~ 100 -
        ((rowMeans(select(., all_of(func)), na.rm = TRUE) * 100) / 4)
    )
  ) %>%
  mutate(
    new_qol_score = case_when(
      qol_not_na >= 2 ~ 100 -
        ((rowMeans(select(., all_of(qol)), na.rm = TRUE) * 100) / 4)
    )
  ) %>%
...
phq <- phq %>%
  ...
  mutate(phq_diff = case_when(phq_not_na >= 6 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
fabq <- fabq %>%
  ...
  mutate(fabq_diff = case_when(fabq_not_na >= 3 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
pcs <- pcs %>%
  ...
  mutate(pcs_diff = case_when(pcs_not_na >= 5 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
pdq <- pdq %>%
  ...
  mutate(pdq_diff = case_when(pdq_not_na >= 5 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
prs <- prs %>%
  ...
  mutate(prs_bhv_diff = case_when(prs_bhv_not_na >= 4 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
prs <- prs %>%
  ...
  mutate(prs_cog_diff = case_when(prs_cog_not_na >= 7 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
prs <- prs %>%
  ...
  mutate(prs_diff = case_when(prs_not_na >= 10 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
misci <- misci %>%
  ...
  mutate(misci_diff = case_when(misci_not_na >= 8 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
dts <- dts %>%
  ...
  mutate(dts_diff = case_when(dts_not_na >= 13 ~ 1, TRUE ~ 0)) %>%
  as_tibble() %>%
...
rapa <- rapa %>%
  mutate(
    rapa_rarly1 = case_when(rapa_rarly == 1 ~ 1, TRUE ~ rapa_rarly),
    rapa_light1 = case_when(rapa_light == 1 ~ 2, TRUE ~ rapa_light),
    rapa_lightweekly1 = case_when(
      rapa_lightweekly == 1 ~ 3,
      TRUE ~ rapa_lightweekly
    ),
    rapa_mod1 = case_when(rapa_mod == 1 ~ 4, TRUE ~ rapa_mod),
    rapa_vig1 = case_when(rapa_vig == 1 ~ 4, TRUE ~ rapa_vig),
    rapa_modweekly1 = case_when(rapa_modweekly == 1 ~ 5, TRUE ~ rapa_modweekly),
    rapa_vigweekly1 = case_when(rapa_vigweekly == 1 ~ 5, TRUE ~ rapa_vigweekly),
