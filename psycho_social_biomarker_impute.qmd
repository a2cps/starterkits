...
# Psychosocial Data Imputation and Biomarker calculations {#sec-psychosocial-methods}
...
psy_soc <- psy_soc |> filter(!record_id %in% test_records)
```
...
```{r}
psy_soc1 <- psy_soc |> select(-all_of(drop_vars))
```
...
We will create a function called 'apply_filter' that subsets the 'psy_soc1' dataset based on field names that match a specified pattern. The function takes two parameters: 'prefix', for form specific field names and 'test_complete_column' for the form specific variable which indicates if the form was complete i.e 2 . It returns a dataset containing the 'record_id', 'redcap_event_name', 'redcap_data_access_group', and the form specific field names for completed forms.

```{r}
apply_filter <- function(prefix, test_complete_column) {
...
Michigan Body Map records pain intensity and duration for the body region indicated by the subject\ [@berardi_multi_2022]. The painful body regions can vary from subject to subject, which can result in null values in field names for body regions not indicated by the subject, this explains the data sparsity.
...
bpi_body_map <- bpi_body_map |>
  ...
  mutate(
    neck = case_when((grepl("a26", bpipainanatsiteareatxt)) ~ 1, .default = 0)
  ) |>
  mutate(
  ...
  mutate(
    chest_or_breast_m = case_when(
      ...
      chest_or_breast == 1 &
        !is.na(bpi_mbm_z3_rate) &
        !is.na(bpi_mbm_z3_dur) ~ 1,
      .default = 2
    )
  ) |>
  mutate(
    abd_pelvis_groin_m = case_when(
      abd_pelvis_groin == 0 &
        is.na(bpi_mbm_z4_rate) &
        is.na(bpi_mbm_z4_dur) ~ 0,
      abd_pelvis_groin == 1 &
        !is.na(bpi_mbm_z4_rate) &
        !is.na(bpi_mbm_z4_dur) ~ 1,
      .default = 2
    )
  ) |>
  mutate(
    right_shoulder_arm_wrist_m = case_when(
      right_shoulder_arm_wrist == 0 &
        ...
        is.na(bpi_mbm_z5_dur) ~ 0,
      right_shoulder_arm_wrist == 1 &
        !is.na(bpi_mbm_z5_rate) &
        !is.na(bpi_mbm_z5_dur) ~ 1,
      .default = 2
    )
  ) |>
  mutate(
    left_shoulder_arm_wrist_m = case_when(
      left_shoulder_arm_wrist == 0 &
        ...
        is.na(bpi_mbm_z6_dur) ~ 0,
      left_shoulder_arm_wrist == 1 &
        !is.na(bpi_mbm_z6_rate) &
        !is.na(bpi_mbm_z6_dur) ~ 1,
      .default = 2
    )
  ) |>
  mutate(
  ...
  mutate(
    right_hip_leg_foot_m = case_when(
      right_hip_leg_foot == 0 &
        ...
        is.na(bpi_mbm_z8_dur) ~ 0,
      right_hip_leg_foot == 1 &
        !is.na(bpi_mbm_z8_rate) &
        !is.na(bpi_mbm_z8_dur) ~ 1,
      .default = 2
    )
  ) |>
  mutate(
    left_hip_leg_foot_m = case_when(
      left_hip_leg_foot == 0 &
        ...
        is.na(bpi_mbm_z9_dur) ~ 0,
      left_hip_leg_foot == 1 &
        !is.na(bpi_mbm_z9_rate) &
        !is.na(bpi_mbm_z9_dur) ~ 1,
      .default = 2
    )
  ) |>
...
bpi_body_pain <- bpi_body_pain |>
  mutate(body_pain_intensity = if_else(is.na(bpiworstpainratingexclss), 0, 1))
```
...
bpi_pain_intrf <- bpi_pain_intrf |>
  mutate(
    intrf_not_na = rowSums(
      !is.na(pick("bpipainintfrgnrlactvtyscl":"bpipainenjoymntintrfrscl"))
    )
  ) |>
  mutate(
...
koos <- koos |>
  mutate(pain_not_na = rowSums(!is.na(pick(all_of(pain))))) |>
  mutate(func_not_na = rowSums(!is.na(pick(all_of(func))))) |>
  mutate(qol_not_na = rowSums(!is.na(pick(all_of(qol))))) |>
  mutate(
    new_pain_score = case_when(
      pain_not_na >= 2 ~ 100 -
        ((rowMeans(pick(all_of(pain)), na.rm = TRUE) * 100) / 4)
    )
  ) |>
  mutate(
    new_func_score = case_when(
      func_not_na >= 2 ~ 100 -
        ((rowMeans(pick(all_of(func)), na.rm = TRUE) * 100) / 4)
    )
  ) |>
  mutate(
    new_qol_score = case_when(
      qol_not_na >= 2 ~ 100 -
        ((rowMeans(pick(all_of(qol)), na.rm = TRUE) * 100) / 4)
    )
  ) |>
  mutate(new_pain_summary = rowMeans(pick(all_of(all_koos))))
```
...
The GAD-7 is a 7-item likert scale questionnaire that assesses anxiety levels over the last two weeks. The response to each item is rated on a scale of 0 ("not at all") to 3 ("nearly every day"). A total score is calculated by adding the scores for all the 7 items and ranges from 0 to 21. A greater total score on the GAD-7 reflects higher anxiety levels\ [@spitzer_brief_2006]. At the end of the questionnaire there is an additional question ("gad7difficulttowork") to evaluate the subject's perception about the impact of identified problems on their activities such as work, taking care of things at home, or interacting with others.
...
gad <- gad |>
  mutate(gad_not_na = rowSums(!is.na(pick(all_of(gad_vector))))) |>
  mutate(
    gad_diff = case_when(
...
gad <- gad |>
  ...
  mutate(
    mean_gad = case_when(
      gad_diff == 1 | gad_diff == 3 ~ round(rowMeans(
        pick(all_of(gad_vector)),
        na.rm = TRUE
      ))
    )
  ) |>
...
```{r}
gad <- gad |> select(-gad_not_na, -mean_gad)
```
...
phq <- phq |>
  mutate(phq_not_na = rowSums(!is.na(pick(all_of(phq_vector))))) |>
  mutate(phq_diff = case_when(phq_not_na >= 6 ~ 1, .default = 0)) |>
  as_tibble() |>
...
phq <- phq |>
  mutate(
    mean_phq = case_when(
      phq_diff == 1 ~ round(rowMeans(
        pick(all_of(phq_vector)),
        na.rm = TRUE
      ))
    )
  ) |>
  ...
  mutate(
    imputed_phq_score = case_when(
      phq_diff == 1 ~ rowSums(pick(all_of(phq_vector)), na.rm = TRUE)
    )
  )
```
...
```{r}
phq <- phq |> select(-phq_not_na, -mean_phq)
```
...
The FABQ is a 4-item questionnaire that assesses fear avoidance behavior. The response to each item ranges from 0 (completely disagree) to 6 (completely agree)\ [@waddell_fear_1993]. A total score is calculated by adding the scores for all the items and ranges from 0-24. A higher score indicates a higher degree of fear-avoidance belief.
...
fabq <- fabq |>
  mutate(fabq_not_na = rowSums(!is.na(pick(all_of(fabq_vector))))) |>
  mutate(fabq_diff = case_when(fabq_not_na >= 3 ~ 1, .default = 0)) |>
  as_tibble() |>
...
fabq <- fabq |>
  mutate(
    raw_fabq_score = rowSums(pick(all_of(fabq_vector)), na.rm = TRUE)
  ) |>
  mutate(
    mean_fabq = case_when(
      fabq_diff == 1 ~ round(rowMeans(
        pick(all_of(fabq_vector)),
        na.rm = TRUE
      ))
    )
  ) |>
  ...
  mutate(
    imputed_fabq_score = case_when(
      fabq_diff == 1 ~ rowSums(pick(all_of(fabq_vector)), na.rm = TRUE)
    )
  )
```
...
```{r}
fabq <- fabq |> select(-fabq_not_na, -mean_fabq)
```
...
```{r}
pcs <- apply_filter("pcq", pain_catastrophizing_questionnaire_pcs6_complete)
```
...
pcs <- pcs |>
  mutate(pcs_not_na = rowSums(!is.na(pick(all_of(pcs_vector))))) |>
  mutate(pcs_diff = case_when(pcs_not_na >= 5 ~ 1, .default = 0)) |>
  as_tibble() |>
...
pcs <- pcs |>
  mutate(
    raw_pcs_score = rowSums(pick(all_of(pcs_vector)), na.rm = TRUE)
  ) |>
  mutate(
    mean_pcs = case_when(
      pcs_diff == 1 ~ round(rowMeans(
        pick(all_of(pcs_vector)),
        na.rm = TRUE
      ))
    )
  ) |>
  ...
  mutate(
    imputed_pcs_score = case_when(
      pcs_diff == 1 ~ rowSums(pick(all_of(pcs_vector)), na.rm = TRUE)
    )
  )
```
...
```{r}
pcs <- pcs |> select(-pcs_not_na, -mean_pcs, -raw_pcs_score)
```
...
```{r}
ssi <- apply_filter("ssi", symptom_severity_index_v10_ssi_complete)
```
...
ssi <- ssi |>
  mutate(
    ssi_symp_score = rowSums(pick(all_of(ssi_symptoms)), na.rm = FALSE)
  ) |>
  mutate(
    ssi_hist_score = rowSums(pick(all_of(ssi_history)), na.rm = FALSE)
  ) |>
  mutate(ssi_total_score = rowSums(pick(all_of(ssi_all)), na.rm = FALSE))
```
...
```{r}
pdq <- apply_filter("pd", pain_detect_questionnaire_pdq_complete)
```
...
pdq <- pdq |>
  mutate(pdq_not_na = rowSums(!is.na(pick(all_of(pdq_seven))))) |>
  mutate(pdq_diff = case_when(pdq_not_na >= 5 ~ 1, .default = 0)) |>
  as_tibble() |>
...
pdq <- pdq |>
  mutate(
    raw_pdq_score = rowSums(pick(all_of(pdq_seven)), na.rm = TRUE)
  ) |>
  mutate(
    mean_pdq = case_when(
      pdq_diff == 1 ~ round(rowMeans(
        pick(all_of(pdq_seven)),
        na.rm = TRUE
      ))
    )
  ) |>
  ...
  mutate(
    imputed_pdq_score = case_when(
      pdq_diff == 1 ~ rowSums(pick(all_of(pdq_seven)), na.rm = TRUE)
    )
  )
```
...
```{r}
pdq <- pdq |> select(-pdq_not_na, -mean_pdq, -raw_pdq_score)
```
...
![](assets/images/Screen%20Shot%202023-08-18%20at%202.59.07%20PM%203-02.png)
...
pdq <- pdq |>
  mutate(
    pd_neuro_total_score = rowSums(pick(all_of(pd_neuro)), na.rm = FALSE)
  ) |>
...
The Sleep II form measures the average number of hours and minutes of sleep each night during the past month\ [@buysse_pittsburgh_1989].
...
```{r}
sleep_dur <- apply_filter("sleep", painsleep_duration_sleep_ii_complete) |>
  retype()
```
...
```{r}
prs <- apply_filter("prs", pain_resilience_scale_prs_complete)
```
...
prs <- prs |>
  mutate(prs_bhv_not_na = rowSums(!is.na(pick(all_of(prs_bhv_five))))) |>
  mutate(prs_bhv_diff = case_when(prs_bhv_not_na >= 4 ~ 1, .default = 0)) |>
  as_tibble() |>
...
prs <- prs |>
  mutate(
    raw_bhv_score = rowSums(pick(all_of(prs_bhv_five)), na.rm = TRUE)
  ) |>
  mutate(
    mean_bhv_prs = case_when(
      prs_bhv_diff == 1 ~ round(rowMeans(
        pick(all_of(prs_bhv_five)),
        na.rm = TRUE
      ))
    )
  ) |>
  ...
  mutate(
    imputed_bhv_score = case_when(
      prs_bhv_diff == 1 ~ rowSums(pick(all_of(prs_bhv_five)), na.rm = TRUE)
    )
  )
```
...
```{r}
prs <- prs |> select(-prs_bhv_not_na, -mean_bhv_prs, -raw_bhv_score)
```
...
prs <- prs |>
  mutate(prs_cog_not_na = rowSums(!is.na(pick(all_of(prs_cog_nine))))) |>
  mutate(prs_cog_diff = case_when(prs_cog_not_na >= 7 ~ 1, .default = 0)) |>
  as_tibble() |>
...
prs <- prs |>
  mutate(
    raw_prs_cog_score = rowSums(pick(all_of(prs_cog_nine)), na.rm = TRUE)
  ) |>
  mutate(
    mean_cog_prs = case_when(
      prs_cog_diff == 1 ~ round(rowMeans(
        pick(all_of(prs_cog_nine)),
        na.rm = TRUE
      ))
    )
  ) |>
  ...
  mutate(
    imputed_prs_cog_score = case_when(
      prs_cog_diff == 1 ~ rowSums(pick(all_of(prs_cog_nine)), na.rm = TRUE)
    )
  )
```
...
```{r}
prs <- prs |> select(-prs_cog_not_na, -mean_cog_prs, -raw_prs_cog_score)
```
...
![](assets/images/Screen%20Shot%202023-08-22%20at%2011.57.31%20AM%203-01.png)
...
prs <- prs |>
  mutate(prs_not_na = rowSums(!is.na(pick(all_of(prs_fourteen))))) |>
  mutate(prs_diff = case_when(prs_not_na >= 10 ~ 1, .default = 0)) |>
  as_tibble() |>
...
prs <- prs |>
  mutate(
    raw_prs_score = rowSums(pick(all_of(prs_fourteen)), na.rm = TRUE)
  ) |>
  mutate(
    mean_prs = case_when(
      prs_diff == 1 ~ round(rowMeans(
        pick(all_of(prs_fourteen)),
        na.rm = TRUE
      ))
    )
  ) |>
  ...
  mutate(
    imputed_prs_score = case_when(
      prs_diff == 1 ~ rowSums(pick(all_of(prs_fourteen)), na.rm = TRUE)
    )
  )
```
...
```{r}
prs <- prs |> select(-prs_not_na, -mean_prs, -raw_prs_score)
```
...
```{r}
misci <- misci |>
  mutate(misci_not_na = rowSums(!is.na(pick(all_of(misci_ten))))) |>
  mutate(misci_diff = case_when(misci_not_na >= 8 ~ 1, .default = 0)) |>
  as_tibble() |>
...
misci <- misci |>
  mutate(
    raw_misci_score = rowSums(pick(all_of(misci_ten)), na.rm = TRUE)
  ) |>
  mutate(
    mean_misci = case_when(
      misci_diff == 1 ~ round(rowMeans(
        pick(all_of(misci_ten)),
        na.rm = TRUE
      ))
    )
  ) |>
  ...
  mutate(
    imputed_misci_score = case_when(
      misci_diff == 1 ~ rowSums(pick(all_of(misci_ten)), na.rm = TRUE)
    )
  )
```
...
```{r}
misci <- misci |> select(-misci_not_na, -mean_misci, -raw_misci_score)
```
...
```{r}
bfi <- apply_filter("bfi", the_big_five_inventory_bfi2s_complete)
```
...
bfi <- bfi |>
  mutate(
    bfi2squietscl_reverse = if_else(
      is.na(bfi2squietscl_reverse),
      bfi2soutgoingscl,
      bfi2squietscl_reverse
    ),
    bfi2soutgoingscl = if_else(
  ...
  mutate(
    bfi2sdominantscl = if_else(
      is.na(bfi2sdominantscl),
      bfi2stakechargescl_reverse,
      bfi2sdominantscl
    ),
    bfi2stakechargescl_reverse = if_else(
  ...
  mutate(
    bfi2senergyscl = if_else(
      is.na(bfi2senergyscl),
      bfi2slessactivescl_reverse,
      bfi2senergyscl
    ),
    bfi2slessactivescl_reverse = if_else(
...
```{r}
bfi <- bfi |> select(-raw_extraversion_score)
```
...
bfi <- bfi |>
  mutate(
    bfi2scompassionatescl = if_else(
      is.na(bfi2scompassionatescl),
      bfi2suncaringscl_reverse,
      bfi2scompassionatescl
    ),
    bfi2suncaringscl_reverse = if_else(
  ...
  mutate(
    bfi2srudescl_reverse = if_else(
      is.na(bfi2srudescl_reverse),
      bfi2srespectfulscl,
      bfi2srudescl_reverse
    ),
    bfi2srespectfulscl = if_else(
  ...
  mutate(
    bfi2sassumingbestscl = if_else(
      is.na(bfi2sassumingbestscl),
      bfi2sfaultwithothersscl_reverse,
      bfi2sassumingbestscl
    ),
    bfi2sfaultwithothersscl_reverse = if_else(
...
```{r}
bfi <- bfi |> select(-raw_agreeableness_score)
```
...
bfi <- bfi |>
  mutate(
    bfi2sdisorganizedscl_reverse = if_else(
      is.na(bfi2sdisorganizedscl_reverse),
      bfi2stidyscl,
      bfi2sdisorganizedscl_reverse
    ),
    bfi2stidyscl = if_else(
      is.na(bfi2stidyscl),
      bfi2sdisorganizedscl_reverse,
      bfi2stidyscl
    )
  ...
  mutate(
    bfi2sdifficultstartscl_reverse = if_else(
      is.na(bfi2sdifficultstartscl_reverse),
      bfi2spersistentscl,
      bfi2sdifficultstartscl_reverse
    ),
    bfi2spersistentscl = if_else(
  ...
  mutate(
    bfi2sreliablescl = if_else(
      is.na(bfi2sreliablescl),
      bfi2scarelessscl_reverse,
      bfi2sreliablescl
    ),
    bfi2scarelessscl_reverse = if_else(
...
bfi <- bfi |>
  mutate(
    bfi2sworriesscl = if_else(
      is.na(bfi2sworriesscl),
      bfi2srelaxedscl_reverse,
      bfi2sworriesscl
    ),
    bfi2srelaxedscl_reverse = if_else(
  ...
  mutate(
    bfi2sdepressedscl = if_else(
      is.na(bfi2sdepressedscl),
      bfi2ssecurescl_reverse,
      bfi2sdepressedscl
    ),
    bfi2ssecurescl_reverse = if_else(
  ...
  mutate(
    bfi2stemperamentalscl = if_else(
      is.na(bfi2stemperamentalscl),
      bfi2sstableemotionscl_reverse,
      bfi2stemperamentalscl
    ),
    bfi2sstableemotionscl_reverse = if_else(
...
```{r}
bfi <- bfi |> select(-raw_emotional_score)
```
...
bfi <- bfi |>
  mutate(
    bfi2sfascinatedscl = if_else(
      is.na(bfi2sfascinatedscl),
      bfi2sfewartinterestscl_reverse,
      bfi2sfascinatedscl
    ),
    bfi2sfewartinterestscl_reverse = if_else(
  ...
  mutate(
    bfi2sllittleinterestscl_reverse = if_else(
      is.na(bfi2sllittleinterestscl_reverse),
      bfi2sthinkerscl,
      bfi2sllittleinterestscl_reverse
    ),
    bfi2sthinkerscl = if_else(
  ...
  mutate(
    bfi2soriginalscl = if_else(
      is.na(bfi2soriginalscl),
      bfi2slittlecreativityscl_reverse,
      bfi2soriginalscl
    ),
    bfi2slittlecreativityscl_reverse = if_else(
...
```{r}
dts <- apply_filter("dts", danish_thoracic_surgery_questionnaire_v02_complete)
```
...
dts <- dts |>
  mutate(dts_not_na = rowSums(!is.na(pick(all_of(dts_vector))))) |>
  mutate(dts_diff = case_when(dts_not_na >= 13 ~ 1, .default = 0)) |>
  as_tibble() |>
...
dts <- dts |>
  mutate(
    raw_dts_score = rowSums(
      pick(all_of(dts_vector)) * (pick(all_of(dts_vector)) != 96),
      na.rm = TRUE
    )
  ) |>
  mutate(
    mean_dts = case_when(
      dts_diff == 1 ~ round(rowMeans(
        pick(all_of(dts_vector)) * (pick(all_of(dts_vector)) != 96),
        na.rm = TRUE
      ))
    )
...
```{r}
dts <- dts |> select(-dts_not_na, -mean_dts, raw_dts_score)
```
...
rapa <- rapa |>
  mutate(
    rapa_rarly1 = case_when(rapa_rarly == 1 ~ 1, .default = rapa_rarly),
    rapa_light1 = case_when(rapa_light == 1 ~ 2, .default = rapa_light),
    rapa_lightweekly1 = case_when(
      rapa_lightweekly == 1 ~ 3,
      .default = rapa_lightweekly
    ),
    rapa_mod1 = case_when(rapa_mod == 1 ~ 4, .default = rapa_mod),
    rapa_vig1 = case_when(rapa_vig == 1 ~ 4, .default = rapa_vig),
    rapa_modweekly1 = case_when(
      rapa_modweekly == 1 ~ 5,
      .default = rapa_modweekly
    ),
    rapa_vigweekly1 = case_when(
      rapa_vigweekly == 1 ~ 5,
      .default = rapa_vigweekly
    ),
...
rapa <- rapa |>
  mutate(
    rapa_aerobic_missing = case_when(
      is.na(rapa_aerobic_score) ~ 1,
      .default = 0
    )
  )
```
