...
# Release-coupled values live in _variables.yml, which is also the source for the
# var shortcodes used elsewhere. They are read here because the URLs below are
# assembled inside R strings, where shortcodes are not expanded.
.vars <- yaml::read_yaml("_variables.yml")
snapshot_tag <- .vars$snapshot$tag
bids_version <- .vars$bids$version
```

This page describes how to work with the raw MRI data from A2CPS. For background on collection and availability, see the "Imaging Data" section of [a2cps.org](https://a2cps.org/researchers/data-releases/about-the-data/). 

The data are provided in a format called the Brain Imaging Data Structure (BIDS)[^bids]. BIDS is a community standard that dictates how neuroimaging data should be organized and named. By enforcing a strict directory structure and naming convention, BIDS ensures that data are machine-readable and easily shareable, which is vital for reproducibility. Raw imaging data are stored as compressed [NIfTI](https://nifti.nimh.nih.gov/) (`.nii.gz`) files, which contain the actual 3D or 4D arrays of brain intensities. Each NIfTI is accompanied by a JSON "sidecar" file (e.g., `.json`) that contains all of the essential metadata about how the scan was acquired (e.g., scanner type, echo time, slice timing) that otherwise would be lost or obscured.

The raw data are most useful when you need to run custom preprocessing pipelines. If you do not need custom pipelines, please review [the preface](index.qmd#sec-preface) for a list of imaging products that have already been calculated.
...
```bash
$ ls image03{{< var release.root >}}/{{< var release.dir >}}/mris/bids/ | head
...
The raw MRI data are organized according to [`{r} bids_version`](`{r} glue::glue("https://bids-specification.readthedocs.io/en/{bids_version}/")`) of the BIDS standard. 
...
**Naming Conventions**:
Under BIDS, you'll see various folders corresponding to the different types of scans acquired:
- `anat`: Structural MRI (sMRI), like T1-weighted (`T1w`) images used for anatomical reference.
- `func`: Functional MRI (fMRI) including both resting-state (rsfMRI; `task-rest`) and task-based (tfMRI; `task-cuff`).
- `dwi`: Diffusion MRI (dMRI) used to measure water diffusion along white matter tracts.
- `fmap`: Contains files used to estimate the magnetic field inhomogeneities, often called "fieldmaps". However, in A2CPS, these are specifically pairs of spin-echo EPI images with reversed phase-encoding directions (AP/PA). They are not literal fieldmaps themselves, but they are used by preprocessing tools (like fMRIPrep or QSIPrep) to estimate and correct for spatial distortions in the functional and diffusion data.
...
The pressures that were applied during the CUFF scans are recorded in the [`sub-\[record_id\]_ses-\[ses_id\]_task-cuff_run-\[run_id\]_events.tsv` tables](`{r} glue::glue("https://bids-specification.readthedocs.io/en/{bids_version}/modality-specific-files/task-events.html")`).

BIDS requires that the diffusion gradient information is stored according to the FSL format, in `*bval` and `*bvec` files in the `/dwi` directory. For details see: [Magnetic Resonance Imaging - Brain Imaging Data Structure](`{r} glue::glue("https://bids-specification.readthedocs.io/en/{bids_version}/modality-specific-files/magnetic-resonance-imaging-data.html#required-gradient-orientation-information")`).
...
tibble::tribble(
  ~File                                                                                                                                                                                                   ,
  ~Documentation                                                                                                                                                                                          ,
  glue::glue("README ([A2CPS file](https://github.com/a2cps/snapshot/blob/{snapshot_tag}/src/snapshot/data/README.md))")                                                                                  ,
  glue::glue("[modality agnostic files](https://bids-specification.readthedocs.io/en/{bids_version}/modality-agnostic-files.html)")                                                                       ,
  glue::glue("participants.tsv ([dictionary](https://github.com/a2cps/snapshot/blob/{snapshot_tag}/src/snapshot/data/participants.json))")                                                                ,
  glue::glue("[participants file](https://bids-specification.readthedocs.io/en/{bids_version}/modality-agnostic-files.html#participants-file)")                                                           ,
  glue::glue("sub-\\[record_id\\]_ses-\\[ses_id\\]_scans.tsv tables ([dictionary](https://github.com/a2cps/snapshot/blob/{snapshot_tag}/src/snapshot/data/scans.json))")                                      ,
  glue::glue("[scans file](https://bids-specification.readthedocs.io/en/{bids_version}/modality-agnostic-files.html#scans-file)")                                                                         ,
  glue::glue("sub-\\[record_id\\]_sessions.tsv tables ([dictionary](https://github.com/a2cps/snapshot/blob/{snapshot_tag}/src/snapshot/data/sessions.json))")                                               ,
  glue::glue("[sessions file](https://bids-specification.readthedocs.io/en/{bids_version}/modality-agnostic-files.html#sessions-file)")                                                                   ,
  glue::glue("sub-\\[record_id\\]_ses-\\[ses_id\\]_task-cuff_run-\\[run_id\\]_events.tsv tables ([dictionary](https://github.com/a2cps/snapshot/blob/{snapshot_tag}/src/snapshot/data/task-cuff_events.json))") ,
  glue::glue("[task events](https://bids-specification.readthedocs.io/en/{bids_version}/modality-specific-files/task-events.html)")                                                                       ,
  glue::glue("dataset_description.json ([A2CPS file](https://github.com/a2cps/snapshot/blob/{snapshot_tag}/src/snapshot/data/dataset_description.json))")                                                 ,
  glue::glue("[dataset descriptions](https://bids-specification.readthedocs.io/en/{bids_version}/modality-agnostic-files.html#dataset_descriptionjson)")
)
```
...
All MRI pipeline code is available through github: <https://github.com/a2cps/mri_imaging_pipeline>. Note that the code used to generate the BIDS derivatives has been updated several times (e.g., the same version of [`dcm2niix`](https://github.com/rordenlab/dcm2niix) was not used to convert all participants). The specific version of the conversion code is stored in the `GeneratedBy` field of the `dataset_description.json`, with entries like
