...
[^imaging-report-help]: Note that by default it includes all collected scans, but you can select subsets like "Release 1" using a menu at the top of the page.
...
The raw MRI data are organized according to [v1.9.0](https://bids-specification.readthedocs.io/en/v1.9.0/) of the Brain Imaging Dataset Structure (BIDS) standard. 
...
The raw imaging data are in the (`gzip` compressed) [NIfTi](https://nifti.nimh.nih.gov/) files, and each image is associated with a `.json` sidecar containing metadata about the scan (metadata that subsumes the NIfTi header). 
...
The pressures that were applied during the CUFF scans are recorded in the [`sub-[record_id]_ses-[ses_id]_task-cuff_run-[run_id]_events.tsv` tables](https://bids-specification.readthedocs.io/en/v1.9.0/modality-specific-files/task-events.html).

BIDS requires that the diffusion gradient information is stored according to the FSL format, in `*bval` and `*bvec` files in the `/dwi` directory. For details see: [Magnetic Resonance Imaging - Brain Imaging Data Structure](https://bids-specification.readthedocs.io/en/v1.9.0/modality-specific-files/magnetic-resonance-imaging-data.html#required-gradient-orientation-information).
...
tibble::tribble(
  ~File, ~Documentation,
  "README ([A2CPS file](https://github.com/a2cps/snapshot/blob/v1.1.0/src/snapshot/data/README.md))", "[modality agnostic files](https://bids-specification.readthedocs.io/en/v1.9.0/modality-agnostic-files.html)",
  "participants.tsv ([dictionary](https://github.com/a2cps/snapshot/blob/f025c754ed77f7fd62ae609224f89d1bc5298fd2/src/snapshot/data/participants.json))", "[participants file](https://bids-specification.readthedocs.io/en/v1.9.0/modality-agnostic-files.html#participants-file )",
  "sub-[record_id]_ses-[ses_id]_scans.tsv tables ([dictionary](https://github.com/a2cps/snapshot/blob/f025c754ed77f7fd62ae609224f89d1bc5298fd2/src/snapshot/data/scans.json))", "[scans file](https://bids-specification.readthedocs.io/en/v1.9.0/modality-agnostic-files.html#scans-file)",
  "sub-[record_id]_sessions.tsv tables ([dictionary](https://github.com/a2cps/snapshot/blob/f025c754ed77f7fd62ae609224f89d1bc5298fd2/src/snapshot/data/sessions.json))","[sessions file](https://bids-specification.readthedocs.io/en/v1.9.0/modality-agnostic-files.html#sessions-file)",
  "sub-[record_id]_ses-[ses_id]_task-cuff_run-[run_id]_events.tsv tables ([dictionary](https://github.com/a2cps/snapshot/blob/f025c754ed77f7fd62ae609224f89d1bc5298fd2/src/snapshot/data/task-cuff_events.json))","[task events](https://bids-specification.readthedocs.io/en/v1.9.0/modality-specific-files/task-events.html)",
  "dataset_description.json ([A2CPS file](https://github.com/a2cps/snapshot/blob/f025c754ed77f7fd62ae609224f89d1bc5298fd2/src/snapshot/data/dataset_description.json))","[dataset descriptions](https://bids-specification.readthedocs.io/en/v1.9.0/modality-agnostic-files.html#dataset_descriptionjson)"
) 
...
Release 2.0.0 (expected by Winter 2025) will include 1017 participants with at least a T1w image. For planned updates, please see the table [Release v2.0](https://a2cps.atlassian.net/wiki/spaces/DOC/pages/177209345) with `Product == bids`.
