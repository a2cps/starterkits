...
The structural T1w scan in the A2CPS neuroimaging protocol is the source of several pre-specified biomarkers. Two of these biomarkers---the volumes of the hippocampi and amygdalae---are derived from a tool in the Functional Magnetic Resonance Imaging of the Brain (FMRIB) Software Library (FSL): [FMRIB's Integrated Registration and Segmentation Tool (FIRST)](https://fsl.fmrib.ox.ac.uk/fsl/docs/structural/first.html). 

Subcortical volumetric measurements are particularly important in pain research. The amygdala plays a critical role in the affective and emotional dimensions of pain processing, while the hippocampus is involved in memory consolidation and contextual fear conditioning associated with painful stimuli. Assessing the volume of these structures can help researchers understand structural adaptations to chronic pain.

The A2CPS pipeline calls FIRST as a part of the FSL standard pipeline for processing anatomical images ([fsl_anat](https://fsl.fmrib.ox.ac.uk/fsl/docs/structural/fsl_anat.html)), which generates several outputs in addition to the biomarkers. This kit provides an overview of the outputs available from that pipeline.
...
```bash
{{< var release.root >}}/{{< var release.dir >}}/mris/derivatives/fslanat
```
...
Many users will not need the raw `fsl_anat` outputs and can instead rely a table in which the subcortical measures have been aggregated: `fslanat.tsv`. A data dictionary for this table is available in the `fslanat.json` file, and also [ online](https://github.com/a2cps/snapshot/blob/{{< var snapshot.tag >}}/src/snapshot/data/fslanat.json).
...
fslanat <- read_tsv(
  "data/pre-surgery/mris/derivatives/fslanat/fslanat.tsv",
...
subcort_fsl <- fslanat |>
  ...
  # too_many = "drop" keeps only the stem: FSL's "Left-Thalamus-Proper" and
  # "Left-Accumbens-area" have to reduce to "Thalamus"/"Accumbens" to line up
  # with the FreeSurfer names below. separate() did this via a suppressed warning.
  separate_wider_delim(
    structure,
    delim = "-",
    names = c("hemisphere", "structure"),
    too_many = "drop"
  )
...
```{r}
#| label: subcort-freesurfer

subcort_freesurfer <- read_tsv(
  "data/pre-surgery/mris/derivatives/freesurfer/aseg.tsv",
  show_col_types = FALSE
) |>
  filter(str_detect(
    StructName,
...
subcort <- bind_rows(
  ...
  drop_na() # not every structure is available in both methods, and not every participant has been processed by both pipelines
...
subcort |>
  ...
  geom_ba() +
  labs(
    x = "Average ([FSL + FreeSurfer]/2)",
    y = "Difference (FSL - FreeSurfer)"
  )
```
...
As expected, there are overall differences in how the estimates provided by the two structures\ [@fig-ba]. We can quantify these differences using the intraclass correlation coefficient.
