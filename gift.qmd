...
In each of these sub-\[record_id\] folders, GIFT outputs the spatial IC maps and timecourses for each run.
...
Many analyses will be based on either these spatial maps or timecourses\ [but see @sec-additional-gift]. For example, let's use these components to search for a possible effect of systematic low-frequency oscillations\ [@tong_low_2019]. These oscillations have been hypothesized to increase the total functional connectivity (that is, the sum of the all connectivity between each pair of components) throughout a scan\ [@korponay_brain_2024]. So, does the moment-by-moment total connectivity increase during a scan?
...
do_cor <- function(x, y, width = 25) {
  ...
  tibble(rho = rho, t = seq_along(x))
}
```

Next, add a helper function for calculating the total correlation:

```{r}
#| label: helper-timecourse

get_total_correlation <- function(file) {
  timecourse <- readNifti(file)
  crossing(src = seq_len(ncol(timecourse)), dst = seq_len(ncol(timecourse))) |>
    filter(dst > src) |>
    mutate(
      correlations = map2(
        src,
        dst,
        \(x, y) do_cor(timecourse[, x], timecourse[, y])
      )
    ) |>
    unnest(correlations) |>
    drop_na() |> # NAs caused by not having enough timepoints for window
...
d <- crossing(
  file = dir_ls(
    "data/pre-surgery/mris/derivatives/gift/derivatives/gift-neuromark_fmri_2.1_modelorder-multi",
    recurse = TRUE,
...
d |>
  ...
  geom_line() +
  labs(x = "Time (seconds)", y = "Total Connectivity")
```
...
The GIFT outputs were generated by the [gift_app](https://github.com/a2cps/mri_imaging_pipeline/tree/master/gift_app) component of the A2CPS Imaging pipeline. Prior to NeuroMark, the resting-state fMRI data were minimally preprocessed (as detailed in the [A2CPS imaging processing pipeline paper](https://www.biorxiv.org/content/10.1101/2024.12.19.627509v1)), and then resampled to 2.4mm isotropic voxel size to ensure identical dimensions across all subjects. After resampling, volumes were spatially smoothed using a Gaussian kernel with a full width at half maximum of 6mm. 

NeuroMark was then applied separately for each resting-state run from each subject and separately for each template. The NeuroMark pipeline included the following configuration options: brain masking using the default mask plus intracranial volume, coregistration of the functional data to a functional template in MNI space (separate from the registration performed during minimal preprocessing), removal of the first 15 volumes (i.e., dummy scans), bandpass filtering (0.01-0.15Hz), removal of the mean per timepoint, multivariate-objective optimization ICA with reference\ [@du_group_2013], and z-score scaling of results. 

The NeuroMark outputs include, separately for each template applied to each resting-state run from each subject, the timecourses and spatial maps of each intrinsic component network, functional network connectivity estimates, summary reports, and visualizations of the networks.
