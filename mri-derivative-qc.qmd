...
```{r session}
read_parquet_duckdb("data/pre-surgery/mris/derivatives/qc/session.parquet") |> head()
```
...
```{r rating}
read_parquet_duckdb("data/pre-surgery/mris/derivatives/qc/rating.parquet") |> head()
```
...
```{r coordinate}
read_parquet_duckdb("data/pre-surgery/mris/derivatives/qc/coordinate.parquet") |> head()
```
...
spatial_norm_images <- read_parquet_duckdb("data/pre-surgery/mris/derivatives/qc/image.parquet") |>
  filter(step == 1, display == 0) |>
  select(id, img)

# grab image with lots of clicks
img_with_many_clicks <- read_parquet_duckdb("data/pre-surgery/mris/derivatives/qc/coordinate.parquet") |>
  filter(!is.na(x)) |>
...
```{r helper}
to_tbl <- function(.i) {
  # apply() already yields hex colours, so they can be handed straight to
  # scale_fill_identity() below; there is no need to split them back into
  # separate channels.
  apply(.i, 2, rgb) |>
    as_tibble() |>
    mutate(y = row_number()) |>
    tidyr::pivot_longer(-y, names_to = "x", values_to = "colour") |>
    mutate(
...
fmap_coregistration_images <- read_parquet_duckdb("data/pre-surgery/mris/derivatives/qc/image.parquet") |>
  filter(step == 3) |>
