...
open_dataset("data/pre-surgery/mris/derivatives/signatures/signatures-by-run") |>
  ...
  rename(SIIPS1 = `137subjmap_weighted_mean`, NPS = `grouppred_cvpcr`)
```
...
pl.read_parquet("data/pre-surgery/mris/derivatives/signatures/signatures-by-run").filter(
    ...
    "sub",
    "ses",
).pivot(on="signature", index=["sub", "ses"]).rename(
