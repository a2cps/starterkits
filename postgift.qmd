...
As a brief primer, extracting functional networks via ICA allows researchers to study how different brain regions communicate. Once these networks are identified, two common metrics are calculated:
1. **Connectivity**: The correlation between the activity (timecourses) of different networks. This tells us how strongly two distinct brain systems are communicating with each other.
2. **Amplitude (fALFF)**: The fractional Amplitude of Low Frequency Fluctuations\ [@zou_improved_2008]. This is a measure of the spontaneous, low-frequency neural activity occurring *within* a specific network, often used to assess baseline brain function and regional abnormalities in conditions like chronic pain.
...
The derivatives are underneath `derivatives/postgift`. Let's take a look.
...
- [amplitude](https://github.com/a2cps/snapshot/blob/main/src/snapshot/data/gift_amplitude.json): a measure of component amplitudes (fALFF).
...
amplitude <- read_parquet_duckdb(
  dir_ls(
    "data/pre-surgery/mris/derivatives/postgift/amplitude",
    recurse = TRUE,
    glob = "*parquet"
  ),
  prudence = "lavish"
...
```{r}
red_run1_scans <- read_tsv(
  dir_ls("data/pre-surgery/mris/bids", glob = "*scans.tsv", recurse = TRUE),
  na = "n/a"
) |>
  filter(rating == "red", str_detect(filename, "cuff_run-0[12]")) |>
  mutate(
    sub = str_extract(filename, "[[:digit:]]{5}") |> as.integer()
  ) |>
  select(sub)
```
...
do_ma_plot <- function(.d) {
  .d |>
    ...
    drop_na() |> # not every participant has a REST1 and CUFF1
...
run1_amplitudes_without_red |> do_ma_plot()
```

Fractional Amplitude of Low Frequency Fluctuations. Each panel is a distinct component. The x-axis is a mean average, and the y-axis is a ratio of the two fALFFs. 
:::
