...
bio <- read_csv(here("data", "pre-surgery", "blood-draw", "raw", "blood-draw-2024-11-06.csv"))
```
...
bio <- bio %>% filter(!record_id %in% test_records)
```
...
![](assets/images/Screenshot%202025-02-03%20at%206.49.01%20PM.png)

**Thoracic**

![](assets/images/Screenshot%202024-12-13%20at%208.16.31%20AM.png)
...
br_report %>%
  gt() %>%
  tab_header(title = md("**TKA Biospecimen collection form Error Report**")) %>%
...
write_csv(
  bio_dict,
  file = here::here("data","pre-surgery", "blood-draw", "Reformatted", "updated_bio_dict.csv")
)
```
