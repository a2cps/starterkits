...
qst <- qst |> filter(!record_id %in% test_records)
```
  ...
  mutate(
    index1 = case_when(
      (!is.na(pptindex1val) & !is.na(pptindex1val1)) |
        ...
        (!is.na(pptindex3val) & !is.na(pptindex3val1)) ~ 1,
      .default = 0
    )
  ) |>
  filter(index1 == 1) |>
  ...
  mutate(
    remote1 = case_when(
      (!is.na(pptremote1val) & !is.na(pptremote1val1)) |
        ...
        (!is.na(pptremote3val) & !is.na(pptremote3val1)) ~ 1,
      .default = 0
    )
  ) |>
  filter(remote1 == 1) |>
...
tka_ts_recoded <- tr.complete |>
  ...
  mutate(
    one_rem = case_when(trrep1 == 1 | trrep2 == 1 | trrep3 ~ 1, .default = 0)
  ) |>
  mutate(
    one_ind = case_when(indrep1 == 1 | indrep2 == 1 | indrep3 ~ 1, .default = 0)
  ) |>
  mutate(one_rep = case_when(one_rem == 1 & one_ind == 1 ~ 1, .default = 0))
```
  ...
  mutate(
    ts.index1 = case_when(
      (!is.na(tsrep1initialpainindex) & !is.na(tsrep1initialpainindex_d)) |
        ...
        (!is.na(tsfinalpainindexsclr3) & !is.na(tsfinalpainindexscl1r3)) ~ 1,
      .default = 0
    )
  ) |>
  filter(ts.index1 == 1) |>
  ...
  mutate(
    ts.bothindex1 = case_when(
      is.na(tsrep1initialpainindex) &
        ...
        is.na(tsfinalpainindexscl1r3) ~ 1,
      .default = 0
    )
  ) |>
  ...
  mutate(
    ts.remote1 = case_when(
      (!is.na(tsrep1initialpainrem) & !is.na(tsrep1initialpainrem_d)) |
        ...
        (!is.na(tsfinalpainremsclr3) & !is.na(tsfinalpainremscl1r3)) ~ 1,
      .default = 0
    )
  ) |>
  filter(ts.remote1 == 1) |>
...
qst_dict <- qst_dict |>
  add_row(!!!primary_ppt_tka_new_row) |>
  add_row(!!!secondary_ppt_tka_new_row) |>
  add_row(!!!primary_ts_index_tka_new_row) |>
  add_row(!!!primary_ts_remote_tka_new_row) |>
  add_row(!!!secondary_ts_index_tka_new_row) |>
  add_row(!!!secondary_ts_remote_tka_new_row) |>
  add_row(!!!primary_cpm_tka_new_row) |>
  add_row(!!!secondary_cpm_tka_new_row)
```
...
qst_report |>
  gt() |>
  tab_header(title = md("**TKA Cohort QST form Error Report**")) |>
  ...
  mutate(
    index1 = case_when(
      (!is.na(pptindxr1val) & !is.na(pptindxr1val_d)) |
        ...
        (!is.na(pptindxr3val) & !is.na(pptindxr3val_d)) ~ 1,
      .default = 0
    )
  ) |>
  filter(index1 == 1) |>
  ...
  mutate(
    remote1 = case_when(
      (!is.na(pptremoter1val) & !is.na(pptremoter1val_d)) |
        ...
        (!is.na(pptremoter3val) & !is.na(pptremoter3val_d)) ~ 1,
      .default = 0
    )
  ) |>
  filter(remote1 == 1) |>
  ...
  mutate(
    ts.index1 = case_when(
      (!is.na(tsindxr1initial) & !is.na(tsindxr1initial_d)) |
        ...
        (!is.na(tsindxr3final) & !is.na(tsindxr3final_d)) ~ 1,
      .default = 0
    )
  ) |>
  filter(ts.index1 == 1) |>
  ...
  mutate(
    ts.remote1 = case_when(
      (!is.na(tsremoter1initial) & !is.na(tsremoter1initial_d)) |
        ...
        (!is.na(tsremoter3final) & !is.na(tsremoter3final_d)) ~ 1,
      .default = 0
    )
  ) |>
  filter(ts.remote1 == 1) |>
...
qst_dict <- qst_dict |>
  add_row(!!!primary_ppt_thor_new_row) |>
  add_row(!!!secondary_ppt_thor_new_row) |>
  add_row(!!!primary_ts_index_thor_new_row) |>
  add_row(!!!primary_ts_remote_thor_new_row) |>
  add_row(!!!secondary_ts_index_thor_new_row) |>
  add_row(!!!secondary_ts_remote_thor_new_row) |>
  add_row(!!!primary_cpm_thor_new_row) |>
  add_row(!!!secondary_cpm_thor_new_row)
```
...
m2qst_report |>
  gt() |>
  tab_header(title = md("**Thoracotomy Cohort QST form Error Report**")) |>
  tab_options(table.font.size = px(12), column_labels.font.size = px(12)) |>
  tab_style(
    style = list(cell_fill(color = "#F4F4F4")),
    locations = cells_body(columns = record_id)
