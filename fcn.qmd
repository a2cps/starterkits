...
```{r dmn}
dmn <- open_dataset("data/fcn/connectivity") |>
  filter(atlas == "dmn") |>
...
```{r}
subs_with_all_runs <- open_dataset("data/fcn/connectivity") |>
...
```{r}
red_fmri <- read_tsv(
  dir_ls("data/rawdata", glob = "*scans.tsv", recurse = TRUE),
  na = "n/a"
) |>
...
```{r}
subs_with_all_runs_ok <- subs_with_all_runs |> anti_join(red_fmri)
```
...
```{r}
fcn <- open_dataset("data/fcn/connectivity") |>
