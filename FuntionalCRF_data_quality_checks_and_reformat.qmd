...
func <- func |> filter(!record_id %in% test_records)
```
...
```{r}
tka_func <- tka_func |> mutate(mep_5tsts = tstspostpainscl - tstsprepainscl)
```
...
func_dict <- func_dict |>
  add_row(!!!walk_mep_new_row) |>
  add_row(!!!tsts_mep_new_row)
```
...
tka_func_report |>
  gt() |>
  tab_header(title = md("**TKA Functional Assessment Error Report**")) |>
...
```{r}
m2frfunc3 <- thor_func |> filter(ftdbctestcmpltyn == 1)
```
