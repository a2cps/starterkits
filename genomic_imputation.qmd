...
Imputation is particularly important in genetic association studies because it allows researchers to identify disease-associated loci that were not directly genotyped, facilitating meta-analyses across different studies that may have used different SNP arrays. For a deeper dive into the methodology and importance of imputation, see the [TOPMed Imputation Server Documentation](https://topmedimpute.readthedocs.io/en/latest/).

Following imputation in this dataset, the number of genetic loci increased from 690,126 to 11,016,319, spanning all 22 autosomes and the X chromosome for 1,375 participants. Prior to imputation, data were preprocessed and formatted for compatibility with the Imputation Server. Post-imputation, we applied rigorous quality control, excluding low-confidence variants and those with a minor allele frequency (MAF) below 1%.
...
To access the variant data efficiently, you should use `plink2` (or `plink` 1.9). Plink can rapidly subset massive genomic datasets to pull only the specific variants or participants you need for an analysis. For more detail on Plink Binary File information and data extraction for genetic variant data, refer to the [Genetic Variant starter kit](genetic_variants.qmd).

## Considerations while working with these data

### Population Structure

Even after rigorous QC, population structure still exists within the dataset. Systematic differences in ancestry between groups can lead to spurious associations in genetic analyses. You should generally include genetic principal components as covariates in any downstream analyses (e.g., Polygenic Risk Scores or Genome-Wide Association Studies).

### Custom Filtering

Researchers can choose to filter out variants based on other, standard QC measures that suit their specific study designs. The data provided here represents a strong baseline, but it may require additional tailoring depending on your research question.

### Data Generation (Imputation Process)
...
*(Note: These files originally had the base name A2CPS_Freeze_2, as reflected in the code below)*

#### Pre-Imputation Data Preparation

Following the Data Preparation Process as outlined here: [TOPMed Imputation Server Data Preparation](https://topmedimpute.readthedocs.io/en/latest/prepare-your-data/)
...
for (chr in chromosomes) {
  ...
  dose[,
    c("AF", "MAF", "AVG_CS", "R2") := {
      af <- sub(".*AF=([^;]*).*", "\\1", INFO)
      maf <- sub(".*MAF=([^;]*).*", "\\1", INFO)
      avg_cs <- sub(".*AVG_CS=([^;]*).*", "\\1", INFO)
      r2 <- sub(".*R2=([^;]*).*", "\\1", INFO)
      list(as.numeric(af), as.numeric(maf), as.numeric(avg_cs), as.numeric(r2))
    }
  ]

  common <- dose[MAF > 0.01]
  well_imputed <- common[R2 > 0.3]
  setnames(well_imputed, "#CHROM", "CHROM", skip_absent = TRUE)
  well_imputed[, chr := sub("^chr", "", CHROM)]
  well_imputed[, start := POS]
  well_imputed[, end := POS]

  keep_ranges <- well_imputed[, .(chr, start, end)]

  fwrite(
    keep_ranges,
    file.path(folder, "keep_ranges.txt"),
    sep = "\t",
    col.names = FALSE,
    quote = FALSE
  )
...
new_fid <- sub("^(.*?)_.*", "\\1", old_iid)
new_iid <- sub(".*?_(.*)", "\\1", old_iid)

update_ids <- data.table(old_fid, old_iid, new_fid, new_iid)
fwrite(
  update_ids,
  "update_ids.txt",
  sep = "\t",
...
# Update Sexes
fam_og <- fread("./A2CPS_RL1-5_For_Imputation_F2.fam")
update_sexes <- data.table(fam_og$V1, fam_og$V2, fam_og$V5)
fwrite(
  update_sexes,
  "update_sexes.txt",
  sep = "\t",
...
fam_now <- fread("./chr1_dose.fam")
update_sexes <- data.table(fam_now$V1, fam_now$V2, fam_og$V5)
fwrite(
  update_sexes,
  "update_sexes_X.txt",
  sep = "\t",
...
fwrite(
  as.data.table(snps_to_exclude),
  "duplicate_snp_list.txt",
  col.names = FALSE
)
```
</details>
