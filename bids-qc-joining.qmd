...
```{r}
aparc <- read_tsv("data/pre-surgery/mris/derivatives/freesurfer/aparc.tsv")
aparc
```
...
```{r}
scan_files <- dir_ls("data/pre-surgery/mris/bids/", glob = "*scans.tsv", recurse = TRUE)
head(scan_files)
```
...
```{r}
aparc_wo_red <- aparc |> anti_join(red_t1w_w_entities, by = join_by(sub, ses))
aparc_wo_red
```
